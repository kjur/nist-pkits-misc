...
- 証明書拡張について
 * ポリシ拡張(certificatePolicies)は+certificatePolicies=[ポリシOID1,ポリシOID2...]+と表ます。
 * ポリシ制約拡張(policyConstraints)のポリシ明示必須(requireExplicitPolicy)フィールドの値は+requireExplicitPolicy=スキップ値+と表ます。
 * ポリシ制約拡張(policyConstraints)のポリシマップ禁止(inhibitPolicyMappping)フィールドの値は+inhibitPolicyMapping=スキップ値+と表ます。
 * anyPolicy禁止拡張(inhibitPolicyMapping)の値は+inhibitAnyPoilcy=スキップ値+と表ます。
 * ポリシマッピング拡張(policyMappings)の値は+policyMappings=[[元ポリシ1,先ポリシ1],...]と表ます。
